...
import { getMetadata, isSpawned } from "../api/token";
...
export const usePPL = () => {
  ...
  const register = async (
    address: string,
    tokenIds: string | string[] | number | number[]
  ) => {
    let success = false;
    let error: Error | null = null;
    if (Array.isArray(tokenIds) && tokenIds.length === 0)
      return (
        openAlert && openAlert("0 tokens are avalibe to register", "warning")
      );
    ...
    try {
      try {
        ...
        if (Array.isArray(tokenIds))
          await pplx.methods
            .registerTokens(
              Array.isArray(address) ? address : tokenIds.map(() => address),
              tokenIds,
              true
            )
            .send(sendArgs);
          ...
          await pplx.methods
            .registerTokens([address], [tokenIds], true)
            .send(sendArgs);
        success = true;
      } catch (err: any) {
    ...
    if (success)
      openAlert &&
        openAlert(
          `Token${Array.isArray(tokenIds) ? "s" : ""} registered!`,
          "info"
        );
  ...
  const claim = async (
    ...
    try {
      try {
        ...
        if (Array.isArray(tokenIds))
          ...
          await pplx.methods
            .claimToTokens([address], [tokenIds])
            .send(sendArgs);
        success = true;
      } catch (err: any) {
        if (err.code && err.code !== -32602) throw err;
    ...
    if (success)
      openAlert &&
        openAlert(
          `Token${Array.isArray(tokenIds) ? "s" : ""} claimed!`,
          "info"
        );
    else if (error) openAlert && openAlert(error.message, "error");
  };
  ...
  const transfer = async (
    ...
    if (Array.isArray(tokenIds) && tokenIds.length === 0)
      return (
        openAlert && openAlert("0 tokens are avalibe to transfer", "warning")
      );
    ...
    try {
      try {
        ...
        if (Array.isArray(tokenIds))
          ...
          await ppl20.methods
            .transferTokens2Account([address], [tokenIds], wallet_address)
            .send(sendArgs);
        success = true;
      } catch (err: any) {
        if (err.code && err.code !== -32602) throw err;
      ...
      if (!success) {
        ...
        if (Array.isArray(tokenIds))
          ...
          await pplx.methods
            .transferTokens2Account([address], [tokenIds], true)
            .send(sendArgs);
      }
    } catch (e) {
      if (e instanceof Error) error = e;
    ...
    if (success)
      openAlert &&
        openAlert(
          `Token${Array.isArray(tokenIds) ? "s" : ""} transfered!`,
          "info"
        );
    else if (error) openAlert && openAlert(error.message, "error");
  };
  ...
  const getAHMCTokens = async (): Promise<{ id: string; src: string }[]> => {
    ...
    for (let i = 0; i < balance; ++i) {
      const tokenId = await ahmc.methods
        .tokenOfOwnerByIndex(eth.account, i)
        .call();
      tIds.push({ id: tokenId, name: name + tokenId });
    }
    for (let tokenId of tIds) {
      try {
        let spawned = await isSpawned(tokenId.id);
        const data = await getMetadata(tokenId.id);
  ...
  const getAHMCToken = async (tokenId: string) => {
    ...
    try {
      let spawned = await isSpawned(tokenId);
    ...
    for (let tokenId of _tids) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          tids.push({
            id: tokenId,
            src: `https://dhsv99qu6u1yj.cloudfront.net/images/${tokenId}.png`,
            name: name + tokenId,
          });
        }
      } catch (err) {
  ...
  const getTokenById = async (tokenId: string) => {
    try {
      ...
      const ahmcMD = await getTokenMetadata(
        config.contract_addresses.ahmc,
        tokenId
      );
      const artwMD = await getTokenMetadata(
        config.contract_addresses.artw,
        tokenId
      );
      if (ahmcT) ahmcT = { ...ahmcT, ...ahmcMD, collection: "ahmc" };
      if (artwT) artwT = { ...artwT, ...artwMD, collection: "artw" };
      if (ahmcT && !artwT) return [ahmcT];
      else if (!ahmcT && artwT) return [artwT];
      else return [ahmcT, artwT];
    } catch (e) {
      console.error(e);
      return [];
    }
