...
import EtheriumDriver from "../lib/EtheriumDriver"
import { useAlert } from "./useAlert"

interface IEtheriumContext {
    account: any
    session: any
    setAccount?: React.Dispatch<React.SetStateAction<any>>
    connect?: () => void
    makeMint?: (ammount: number) => void
}

export const EtheriumContext = React.createContext<IEtheriumContext>({
    account: null,
    session: null,
})

export const EtheriumProvider: React.FC = ({ children }) => {
    ...
    const createSession = () => {
        const session = new EthereumSession({
            ...
            contractAddress: '0x61DB9Dde04F78fD55B0B4331a3d148073A101850',
            contractABI: [
    ...
    const makeMint = async (ammount: number) => {
        ...
        try {
            const account = session.wallet.accounts[0];
            setAccount(account)
            const price = await session.contract.methods.price().call();
            const priceBN = Web3.utils.toBN(price);

            const quantityBN = Web3.utils.toBN(ammount);

            const valueBN = quantityBN.mul(priceBN);
            await session.contract.methods.mint(quantityBN.toString()).send({ 'from': account, 'value': valueBN.toString() });
        } catch (error) {
            console.error(error)
    ...
    return <EtheriumContext.Provider value={{
        session,
        account,
        makeMint,
        connect
...
export const useEtherium = () => {
    return useContext(EtheriumContext)
}